...
- Before opening a Pull Request, an AI agent must present a summary of the work done and wait for explicit human confirmation before opening it.
- The Pull Request description must not contain AI-tool attribution/promotional footers (e.g. "Generated by Claude Code", tool links, session links). If a tool auto-appends such a footer, the agent must strip it before finishing the task.
